...
def collect_eu_weather_data(delay_seconds: int = 1) -> Dict:
    ...
    for i, cap in enumerate(EU_CAPITALS):
        city = cap["city"]
        country = cap["country"]
        progress = ((i+1) / len(EU_CAPITALS)) * 100
        print(f"> Fetching: {city} ({i+1}/{len(EU_CAPITALS)}) [{progress:.0f}%]")
...
def main():
    ...
    output_file = "eu_weather_data.json"  # Fixed filename to match requirements
    ...
    if temps:
        ...
        # Filter data with valid temperatures for min/max
        valid_data = [d for d in all_data.values() 
                      if d["current_weather"]["temperature"] is not None]
        
        if valid_data:
            hottest = max(valid_data, key=lambda x: x["current_weather"]["temperature"])
            coldest = min(valid_data, key=lambda x: x["current_weather"]["temperature"])
